...
(function () {
  ...
  const App = {
    ...
    actions: {
      resetEventForm() {
        App.state.editingEventId = null; if (App.els.eventNameInput) App.els.eventNameInput.value = ''; if (App.els.eventColorInput) App.els.eventColorInput.value = '#1f7a45'; if (App.els.eventAddressInput) App.els.eventAddressInput.value = ''; if (App.els.eventScheduleInput) App.els.eventScheduleInput.value = '';
      },
      saveEventTemplate() {
    ...
    ui: {
      ...
      renderEvents() {
        if (App.els.eventsList) App.els.eventsList.innerHTML = App.state.app.events.map((event) => `<div class="event-row"><div><strong>${App.utils.escapeHtml(event.name)}</strong><div class="small">${App.utils.escapeHtml(event.schedule || App.utils.t('no_schedule'))}</div><div class="small">${event.address ? `<a href="${App.utils.mapUrl(event.address)}" target="_blank" rel="noopener noreferrer">${App.utils.escapeHtml(event.address)}</a>` : App.utils.escapeHtml(App.utils.t('no_address'))}</div></div><div style="display:grid;gap:8px;justify-items:end"><span class="pill"><span class="dot" style="background:${event.color}"></span>${event.color}</span><button class="btn" type="button" data-edit-event="${event.id}">${App.utils.t('edit')}</button></div></div>`).join('') || `<div class="empty">${App.utils.t('no_events_month')}</div>`;
        document.querySelectorAll('[data-edit-event]').forEach((btn) => btn.addEventListener('click', () => { const event = App.data.getEventById(btn.dataset.editEvent); App.state.editingEventId = event?.id || null; if (App.els.eventNameInput) App.els.eventNameInput.value = event?.name || ''; if (App.els.eventColorInput) App.els.eventColorInput.value = event?.color || '#1f7a45'; if (App.els.eventAddressInput) App.els.eventAddressInput.value = event?.address || ''; if (App.els.eventScheduleInput) App.els.eventScheduleInput.value = event?.schedule || ''; }));
      },
      renderNotes() {
        const query = (App.state.noteSearch || '').trim().toLowerCase(); const items = []; Object.keys(App.state.app.serviceYears).sort((a,b) => Number(a) - Number(b)).forEach((year) => { Object.values(App.state.app.serviceYears[year].weeks || {}).forEach((week) => { if (!week.note) return; const event = App.data.getEventById(week.eventId); const haystack = `${week.note} ${event?.name || ''} ${week.weekId}`.toLowerCase(); if (query && !haystack.includes(query)) return; items.push({ year, week, event }); }); });
        if (App.els.notesList) App.els.notesList.innerHTML = items.map(({ year, week, event }) => `<div class="note-row"><div><strong>${App.utils.serviceYearLabel(Number(year))}</strong><div class="small">${App.utils.prettyDate(week.start)} — ${App.utils.prettyDate(week.end)} · ${App.utils.escapeHtml(event?.name || App.utils.t('no_template'))}</div><div style="margin-top:8px">${App.utils.escapeHtml(week.note)}</div></div><button class="btn" type="button" data-jump-week="${week.weekId}" data-jump-year="${year}">${App.utils.t('open')}</button></div>`).join('') || `<div class="empty">${App.utils.t('no_notes')}</div>`;
        document.querySelectorAll('[data-jump-week]').forEach((btn) => btn.addEventListener('click', () => { App.state.selectedScreen = 'weeks'; App.state.selectedYear = Number(btn.dataset.jumpYear); App.state.selectedWeekId = btn.dataset.jumpWeek; App.ui.renderAll(); }));
      },
